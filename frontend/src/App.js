    ...
    <div style={{ marginBottom: 22 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <input
          ...
          disabled={locked}
          style={{
            ...inputStyle, flex: 1, opacity: locked ? 0.7 : 1,
...
// ── MVP Entry Block (FIXED - moved outside MvpForm) ───────────────────────────
  ...
  return (
    <div>
      ...
      <p style={{ color: "#666", fontSize: 13, marginBottom: 16 }}>
        ...
        <a href="https://discord.com/channels/1150539550561681508/1150539551517978676"
           target="_blank" rel="noopener noreferrer" style={{ color: "#4da6ff" }}>
          Staff Applicants
        </a>
      </p>
        ...
        <div key={i} style={{ ...cardStyle, marginBottom: 16 }}>
          ...
          <div>
            <label style={{ display: "block", marginBottom: 4 }}>
              ...
              {entry.name.trim() && <span style={{ color: "#ff6b6b", fontSize: 11, fontStyle: "italic" }}> *Required</span>}
              <span style={{ display: "block", color: "#555", fontSize: 11, marginTop: 2 }}>
            ...
            <textarea
              ...
              style={{
                ...inputStyle, resize: "vertical", fontFamily: "'Crimson Pro', serif",
                fontSize: 13, lineHeight: 1.6,
                borderColor: entry.name.trim() && !entry.feedback.trim() ? "rgba(255,107,107,0.45)" : "rgba(255,255,255,0.13)",
              }}
            />
          </div>
        </div>
...
function MvpForm({ pollData, onRefresh }) {
  ...
  const [staffEntries, setStaffEntries] = useState([
    { name: "", feedback: "" }, { name: "", feedback: "" }, { name: "", feedback: "" }
  ]);
      ...
      {mvp.image && (
        <div style={{ marginBottom: 20 }}>
          <img src={mvp.image} alt="MVP banner" style={{
            ...
            border: "1px solid rgba(255,215,0,0.15)", boxShadow: "0 4px 24px rgba(0,0,0,0.5)",
...
// ── Results Panels, Admin Panels (Restored) ───────────────────────────────────
// Due to extremely long message, the remaining panels (ResultsPanel, ApplicantResultsPanel, etc.) are the same as your original.
// Replace the placeholder below with your original code if needed, or let me know if you want me to send them separately.
      ...
      {results.map((r, i) => (
        <div key={r.name} style={{ ...cardStyle, marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
            <span style={{ fontFamily: "'Cinzel',serif", fontSize: 18, color: i === 0 ? "#60a5fa" : "#555", width: 24, flexShrink: 0 }}>
              {i === 0 ? "★" : i + 1}
            </span>
    ...
    <div>
      ...
      {mvp.staffEnabled && (
        <div style={{ marginBottom: 24 }}>
          ...
          {staffResults.length === 0 ? <div style={{ color: "#555", fontSize: 13 }}>No picks yet.</div> : <ResultList results={staffResults} colorKey="staff" />}
        </div>
      )}
      {mvp.adminEnabled && (
        <div style={{ marginBottom: 24 }}>
          <div style={sectionHeaderStyle}>👑 {monthLabel}Admin MVP</div>
          {adminResults.length === 0 ? <div style={{ color: "#555", fontSize: 13 }}>No picks yet.</div> : <ResultList results={adminResults} colorKey="admin" />}
        </div>
      )}
      {!mvp.staffEnabled && !mvp.adminEnabled && (
...
function VoteEditorPanel({ pollData, adminPassword, onRefresh }) {
  ...
  const startEdit = key => {
    setEditingKey(key);
    setEditData(JSON.parse(JSON.stringify(votes[key])));
    setMsg("");
  };
        ...
        <div>
          ...
          {picks.map((pick, i) => (
            <div key={i} style={{ ...cardStyle, marginBottom: 10 }}>
              ...
              <input value={pick.name || ""} onChange={e => setEditData(d => { const p = [...d.picks]; p[i] = { ...p[i], name: e.target.value }; return { ...d, picks: p }; })} placeholder="Name" style={{ ...inputStyle, marginBottom: 8, fontSize: 12 }} />
              <textarea value={pick.feedback || ""} onChange={e => setEditData(d => { const p = [...d.picks]; p[i] = { ...p[i], feedback: e.target.value }; return { ...d, picks: p }; })} rows={2} placeholder="Feedback" style={{ ...inputStyle, resize: "vertical", fontSize: 12, fontFamily: "'Crimson Pro',serif" }} />
              <button onClick={() => setEditData(d => ({ ...d, picks: d.picks.filter((_, idx) => idx !== i) }))} style={{ ...removeBtnStyle, marginTop: 6, fontSize: 11 }}>✕ Remove</button>
            </div>
          ))}
          {picks.length < 3 && (
            <button onClick={() => setEditData(d => ({ ...d, picks: [...(d.picks || []), { name: "", feedback: "" }] }))} style={{ ...addBtnStyle, fontSize: 12, padding: "7px 14px" }}>+ Add Pick</button>
          )}
        </div>
      );
    }
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {picks.map((pick, i) => (
              <div key={i} style={{ ...cardStyle, marginBottom: 8 }}>
                <input value={pick.name || ""} onChange={e => setEditData(d => { const p = [...(d[key] || [])]; p[i] = { ...p[i], name: e.target.value }; return { ...d, [key]: p }; })} placeholder="Name" style={{ ...inputStyle, marginBottom: 8, fontSize: 12 }} />
                <textarea value={pick.feedback || ""} onChange={e => setEditData(d => { const p = [...(d[key] || [])]; p[i] = { ...p[i], feedback: e.target.value }; return { ...d, [key]: p }; })} rows={2} placeholder="Feedback" style={{ ...inputStyle, resize: "vertical", fontSize: 12, fontFamily: "'Crimson Pro',serif" }} />
                <button onClick={() => setEditData(d => ({ ...d, [key]: (d[key] || []).filter((_, idx) => idx !== i) }))} style={{ ...removeBtnStyle, marginTop: 6, fontSize: 11 }}>✕ Remove</button>
              </div>
            ))}
            <button onClick={() => setEditData(d => ({ ...d, [key]: [...(d[key] || []), { name: "", feedback: "" }] }))} style={{ ...addBtnStyle, fontSize: 12, padding: "7px 14px" }}>+ Add</button>
          </div>
        );
      };
      return (
  ...
  return (
    <div>
      ...
      {voterKeys.length === 0 && <div style={{ textAlign: "center", padding: "30px 0", color: "#555", fontSize: 14 }}>No submissions recorded yet.</div>}
      ...
      {!editingKey && voterKeys.map(key => (
        <div key={key} style={{ ...cardStyle, display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
          ...
          {voteType === "staff" && <span style={{ fontSize: 11, color: "#666", flex: 2 }}>{Object.keys(votes[key]?.feedbacks || {}).length} feedback entries</span>}
          {voteType === "mvp" && <span style={{ fontSize: 11, color: "#666", flex: 2 }}>S: {(votes[key]?.staffPicks||[]).map(p=>p.name||p).join(", ")||"—"} | A: {(votes[key]?.adminPicks||[]).map(p=>p.name||p).join(", ")||"—"}</span>}
          {voteType === "applicant" && <span style={{ fontSize: 11, color: "#666", flex: 2 }}>{(votes[key]?.picks||[]).map(p=>p.name||p).join(", ")||"—"}</span>}
          <button onClick={() => startEdit(key)} style={{ padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12, background: "rgba(245,197,66,0.1)", border: "1px solid rgba(245,197,66,0.3)", color: "#f5c542" }}>✏ Edit</button>
          <button onClick={() => deleteVote(key)} disabled={saving} style={{ padding: "5px 10px", borderRadius: 6, cursor: "pointer", fontSize: 12, background: "#ff444418", border: "1px solid #ff4444", color: "#ff8888" }}>✕</button>
        </div>
      ))}
      ...
      {editingKey && (
        <div style={{ background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,215,0,0.15)", borderRadius: 12, padding: "18px 16px", marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
            <button onClick={saveEdit} disabled={saving} style={{ ...submitBtnStyle, flex: 1 }}>{saving ? "Saving…" : "Save Changes"}</button>
            <button onClick={cancelEdit} style={{ flex: 1, padding: "13px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "#888", cursor: "pointer", fontFamily: "'Cinzel',serif", fontWeight: 700, fontSize: 13 }}>Cancel</button>
          </div>
        </div>
      )}
    </div>
...
// ── ToggleSwitch & SettingsPanel & AdminPanel ─────────────────────────────────
...
function SettingsPanel({ pollData, adminPassword, onRefresh }) {
  ...
  const addStaffCand = () => { 
    ...
    setStaffCandidates(c => [...c, newStaffName.trim()]); 
    setNewStaffName(""); 
  };
  ...
  const addAdminCand = () => { 
    ...
    setAdminCandidates(c => [...c, newAdminName.trim()]); 
    setNewAdminName(""); 
  };
  ...
  const addAppCand = () => { 
    ...
    setAppCandidates(c => [...c, newAppName.trim()]); 
    setNewAppName(""); 
  };
    ...
    <div>
      ...
      <div style={{ borderTop: "1px solid rgba(255,215,0,0.12)", paddingTop: 24, marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 32 }}>
          <input 
            ...
            style={{ ...inputStyle, flex: 1 }} 
            onKeyDown={e => e.key === "Enter" && addAppCand()} 
          />
      ...
      <div style={{ borderTop: "1px solid rgba(255,215,0,0.12)", paddingTop: 24, marginBottom: 8 }}>
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <input 
            value={mvpMonth} 
            onChange={e => setMvpMonth(e.target.value)} 
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {staffEnabled && (
            <>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input value={newStaffName} onChange={e => setNewStaffName(e.target.value)} placeholder="Add candidate name" style={{ ...inputStyle, flex: 1 }} onKeyDown={e => e.key === "Enter" && addStaffCand()} />
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {adminEnabled && (
            <>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input value={newAdminName} onChange={e => setNewAdminName(e.target.value)} placeholder="Add candidate name" style={{ ...inputStyle, flex: 1 }} onKeyDown={e => e.key === "Enter" && addAdminCand()} />
